...
class InfoMentorAuth:
	...
	async def _get_oauth_token(self) -> Optional[str]:
		...
		async with self.session.get(HUB_BASE_URL, headers=DEFAULT_HEADERS) as resp:
		...
		location = HUB_BASE_URL + location_match.group(1).strip()
		_LOGGER.debug(f"Following redirect to: {location}")
		
		# Follow redirect to get OAuth token
		async with self.session.get(location, headers=DEFAULT_HEADERS) as resp:
			text = await resp.text()
			
		# Extract OAuth token using regex pattern from original script
		oauth_match = re.search(r'oauth_token" value="([\w+=/]+)"', text)
		if oauth_match:
	...
	async def _send_credentials(self, username: str, password: str) -> None:
		...
		# Build authentication payload similar to data.json
		auth_data = self._build_auth_payload(username, password)
	...
	async def _get_callback_url(self, oauth_token: str) -> Optional[str]:
		...
		# Send OAuth token to get callback URL
		headers.update({
			"Content-Type": "application/x-www-form-urlencoded",
			"Origin": HUB_BASE_URL,
			"Referer": login_url,
